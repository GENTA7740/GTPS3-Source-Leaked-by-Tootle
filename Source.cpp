			...
			try {
				...
				case ENET_EVENT_TYPE_RECEIVE:
				{
					...
					case 3: // world/enter
					{
						...
						if (cch == "action|quit") { // kai quit issaugo dar bus settings ar captcha bypassed disconnect
							...
							try {
								...
								cout << "World klaida -> " << klaida.what() << endl;
							}
						}
						break;
					}
					case 4:
					{
						...
						PlayerMoving* p_ = unpackPlayerMoving(get_struct(event.packet));
											...
											for (int i_ = 0; i_ < it->second.size(); i_++) {
												try {
													...
													if ((int)npc_.kryptis == 180) { // check if it wasnt blocked
														...
														if (items.at(world_->blocks.at((int)(current_x / 32) + ((int)(current_y / 32) * 100)).fg).collisionType != 1) {
														...
														for (int i2 = 0; i2 < new_tiles.size(); i2++) {
															...
															if (x_ < 99 and items.at(shadow_copy.at(x_ + 1 + (y_ * 100)).fg).collisionType != 1) {
																if (not shadow_copy.at(x_ + 1 + (y_ * 100)).scanned) {
																	shadow_copy.at(x_ + 1 + (y_ * 100)).scanned = true;
																	new_tiles.push_back(x_ + 1 + (y_ * 100));
																}
															}
															else if (items.at(shadow_copy.at(x_ + 1 + (y_ * 100)).fg).collisionType == 1 and x_ < npc_.x) {
																blocked_ = true;
																break;
															}
														}
